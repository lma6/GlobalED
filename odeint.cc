...
int patch::check_for_negatives(double dt){
   ...
   if ((water<0)|(fast_soil_C<0)|(structural_soil_C<0)|(slow_soil_C<0)|(mineralized_soil_N<0)|(fast_soil_N<0)|(structural_soil_L<0)) return 1;
   
   if ((old_water+dwdt*dt<0)|(old_fast_soil_C+dfsc*dt<0)|(old_structural_soil_C+dstsc*dt<0)|(old_slow_soil_C+dssc*dt<0)|(old_mineralized_soil_N+dmsn*dt<0)|(old_fast_soil_N+dfsn*dt<0)|(old_passive_soil_C+dpsc*dt<0)|(old_structural_soil_L+dstsl*dt<0)) return 1;
   
   if (water+dwdt*dt<0) return 1; //update_water uses old_water, not water, so check for negatives there as well.
   
   cohort* cc = shortest;
   while (cc != NULL) {
      if ((cc->dndt+cc->ddbhdt+cc->dbalivedt+cc->dbdeaddt)*0!=0) return 2;
      if ((cc->nindivs<0)|(cc->dbh<0)|(cc->balive<0)|(cc->bdead<0)) return 1;
      if ((cc->nindivs+cc->dndt*dt<0)|(cc->dbh+cc->ddbhdt*dt<0)|(cc->balive+cc->dbalivedt*dt<0)|(cc->bdead+cc->dbdeaddt*dt<0)) return 1;  
      cc = cc->taller;
   }
   return 0;
}
